...
/*------------Слайдер "Статьи"---*/
const blogSliderBox = document.querySelector('.blog-slider');
...
if (blogSliderBox) {
   ...
   const swiperPrevBlog = document.getElementById('blogPrev');
   const swiperNextBlog = document.getElementById('blogNext');
   if (swiperPrevBlog) {
      swiperPrevBlog.addEventListener('click', () => {
         blogSlider.slidePrev();
      });
   }
   if (swiperNextBlog) {
      swiperNextBlog.addEventListener('click', () => {
         blogSlider.slideNext();
      });
   }

   // Функция для выравнивания высоты всех слайдов по самому высокому
   function setEqualSlideHeightBlog() {
      const slides = document.querySelectorAll('.blog-slider__wrapper .swiper-slide');
      if (slides.length === 0) return;
   ...
   // Запускаем при загрузке
   window.addEventListener('load', setEqualSlideHeightBlog);
   // И при изменении размера окна (адаптив)
   window.addEventListener('resize', setEqualSlideHeightBlog);

   // Небольшая задержка на случай асинхронной подгрузки шрифтов и т.п.
   setTimeout(setEqualSlideHeightBlog, 100);
